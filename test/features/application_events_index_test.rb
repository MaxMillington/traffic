...
module TrafficSpy
  class ApplicationEventsIndexTest < Minitest::Test
    ...
    def test_most_received_event_to_least_received_is_displayed_on_event_page
      populate
      visit '/sources/jumpstartlab/events'
      within('#list-of-events') do
        assert page.has_css?('li', count: 3)
        assert page.has_content?('application: 3')
        assert page.has_content?('socialLogin: 2')
        assert page.has_content?('otherLogin: 1')
      end
    end

    def test_hyperlinks_to_view_event_specific_data_are_displayed_on_event_page
      populate  
      visit '/sources/jumpstartlab/events'
      within('#list-of-events') do
        hrefs = all('li a').map { |a| a[:href] }
        assert hrefs.include?("/sources/jumpstartlab/events/application")
        assert hrefs.include?("/sources/jumpstartlab/events/otherLogin")
        assert hrefs.include?("/sources/jumpstartlab/events/socialLogin")
      end
    end
